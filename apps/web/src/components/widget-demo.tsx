...
export function WidgetDemo() {
  ...
  const lifiWidgetKey = `${connectedAddress ?? "disconnected"}:${String(
    account.chainId ?? chainId ?? "no-chain",
  ...
  const isTurnkeyActive = activeConnector?.id === "turnkey";
    ...
    <div className="route-layout stack">
      <header className="route-header">
        <div className="stack">
          ...
          <h1>LI.FI widget recognition across Turnkey and third-party wallets.</h1>
          <p className="text-muted">
            The Turnkey embedded wallet can auto-connect into Wagmi after auth, and Reown AppKit
            can replace it with an external wallet. When Turnkey is offline, the app should still
            recognize any active third-party Wagmi wallet and use it as-is.
          </p>
        </div>
      ...
      {sessionGate.authState === "unauthenticated" && account.isConnected ? (
        <div className="session-banner">
          Turnkey is unauthenticated. The connected Wagmi wallet remains usable, but Turnkey-only
          embedded actions stay unavailable until you sign in again.
        </div>
      ) : null}

      {sessionGate.reconnectRequired ? (
        <div className="session-banner">
          Turnkey session is no longer valid. Turnkey-backed actions will pause until you reconnect.
      ...
      <section className="dashboard-grid">
        <article className="panel dashboard-panel stack">
              ...
              <button
                ...
                onClick={() => {
                  void switchChain(chain.id)
                    ...
                    .catch((cause) => {
                      setError(
                        cause instanceof Error ? cause.message : "Chain switch failed.",
                      );
                    });
                }}
                type="button"
          ...
          <div className="status-grid">
            ...
            <div className="status-card">
              <span className="status-label">Wallet mode</span>
              <div className="status-value">
                {isTurnkeyActive ? "turnkey embedded" : connectedAddress ? "external wallet" : "none"}
              </div>
            </div>
            <div className="status-card">
            ...
            <div className="status-card">
              <span className="status-label">Active address</span>
              <div className="status-value">{account.address ?? "not connected"}</div>
            </div>
            <div className="status-card">
          ...
          {balanceError ? (
            <div className="session-banner">Native balance fetch failed: {balanceError.message}</div>
          ) : null}

          <div className="stack">
            <h2>Wagmi signer smoke test</h2>
            <p>
              This signs a simple message through the currently selected Wagmi connector. If the
              Turnkey connector is active, the signature should come from the embedded wallet. If
              an external wallet is active, the same Wagmi action should follow that connector
              instead, even without a live Turnkey session.
            </p>
            <div className="button-row">
              <button
                className="action-button primary"
        ...
        <article className="panel dashboard-panel widget-shell stack">
          ...
          <p>
            The widget lives in the same provider tree. If this route is working correctly, LI.FI
            should treat the active Wagmi wallet as already connected whether it came from Turnkey
          ...
          {connectedAddress ? (
            ...
            <div className="session-banner">Connect a wallet to enable LI.FI route fetching.</div>
          )}
        </article>
      </section>
    </div>
